...
export class MicrophoneRecorder extends BaseScriptComponent {
  ...
  // Configure communication analysis (call this to set API key and options)
  configureCommunicationAnalysis(apiKey: string, focusAreas: string[] = ['general']) {
    if (!isNull(this.communicationAnalyzer)) {
      this.communicationAnalyzer.configure({
        apiKey: apiKey,
  ...
  getCommunicationFeedback(): CommunicationFeedback | null {
    return this.communicationFeedback;
  }

  // Check if communication analysis is available
  hasCommunicationFeedback(): boolean {
    return this.communicationFeedback !== null;
  }

  // Toggle communication analysis
  toggleCommunicationAnalysis(enabled: boolean) {
    this.enableCommunicationAnalysis = enabled;
  }

  // Set analysis focus areas
  setAnalysisFocusAreas(areas: string) {
    this.analysisFocusAreas = areas;
  }

  // Get quick communication summary
  getQuickCommunicationSummary(): string {
    if (!this.communicationFeedback) {
      return "No analysis available";
    }
    
    const score = this.communicationFeedback.overallScore;
    const emoji = score >= 80 ? "🌟" : score >= 60 ? "👍" : "💪";
    return `${emoji} Score: ${score}/100 - ${this.communicationFeedback.summary}`;
  }

  // Show communication feedback manually
  showCommunicationFeedback() {
    if (!this.communicationFeedback) {
      print("No communication feedback available");
      return;
    }
  ...
  // Hide communication feedback
  hideCommunicationFeedback() {
    if (!isNull(this.feedbackDisplay)) {
      this.feedbackDisplay.hideFeedback();
    }
  }
